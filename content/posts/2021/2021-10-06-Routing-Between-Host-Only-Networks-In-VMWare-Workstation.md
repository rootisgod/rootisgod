...
The obvious solution is to go to the cloud, and use the various VNET and firewall controls they provide to do this. The downside is that this gets expensive pretty quickly... Try creating a few VMs (even then powered off!) and see how disk space is still pretty expensive. And, it still makes testing quite laborious. If you make a mistake then reverting to a previous state still isn't that simple and creates more cost on disk usage, backup vaults etc.... It's a bit too inflexible compare to a VM snapshot....

So, enter VMWare Workstation, the best desktop hypervisor by a mile. Just try a demo and you wont go back to Virtualbox anytime soon. So, we can create two VMs on our local machine and try an accomplish a split network setup. Simple, right? But, it gets trickier than I though in this area. While we do get some network tools, it's not quite the level of something like Azure and the ability to create VNETs, subnets and NSGs as we please. VMWare Workstation offers 3 network modes;
